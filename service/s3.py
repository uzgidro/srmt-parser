...
REFERENCE_KEYWORDS = [
    "Андижон", "Оҳангарон", "Ҳисорак", "Тўполанг", "Чирчиқ", "Чорвоқ"
]
MATCH_THRESHOLD = 70
...
def get_object_name(filepath: str):
    mapping = {
        "Андижон": (2, "Andijon"),
        "Оҳангарон": (5, "Ohangaron"),
        "Ҳисорак": (4, "Hisorak"),
        "Тўполанг": (3, "To'palang"),
        "Чорвоқ": (1, "Chorvoq"),
        "Чирчиқ": (1, "Chorvoq")
    }
    ...
    best_match, score = process.extractOne(filepath.rsplit('/', 1)[0], mapping.keys())

    if score >= MATCH_THRESHOLD:
        position, name = mapping[best_match]
...
def get_required_files_from_rar(rar_file_bytes):
    ...
    with rarfile.RarFile(rar_file_bytes) as rf:
        for info in rf.infolist():
            ...
            best_match, score = process.extractOne(filepath.rsplit('/', 1)[0], REFERENCE_KEYWORDS)
            if score >= MATCH_THRESHOLD:
